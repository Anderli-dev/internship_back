...
async def get_company(company_id: int, db: AsyncSession = Depends(get_db)):
    ...
    company = await CompanyReadService().read_company_from_db(db, company_id)
    logger.info(f"Company with ID {company_id} fetched successfully")
    return company


@router.post("/", response_model=CompanyResponse)
async def create_company(company_data: CompanyCreate, db: AsyncSession = Depends(get_db), payload: dict = Depends(Auth().get_token_payload)):
    ...
    company = await CompanyCreateService.create_company(db, company_data, user.id)
...
async def update_company(company_id: int, company_data: CompanyUpdate, db: AsyncSession = Depends(get_db), payload: dict = Depends(Auth().get_token_payload)):
    logger.info(f"Updating company with ID: {company_id}")
    user: User = await UserReadService().read_auth_user(db, payload=payload)
    logger.info(f"Authenticated user ID: {user.id}")

    updated_company = await CompanyUpdateService.update_company(db, company_id, company_data, user_id=user.id)
    logger.info(f"Company with ID {company_id} updated successfully by user ID {user.id}")
    return updated_company


@router.delete("/{company_id}")
async def delete_company(company_id: int, db: AsyncSession = Depends(get_db), payload: dict = Depends(Auth().get_token_payload)):
    logger.info(f"Deleting company with ID: {company_id}")
    user: User = await UserReadService().read_auth_user(db, payload=payload)
    logger.info(f"Authenticated user ID: {user.id}")

    result = await CompanyDeleteService.delete_company(db, company_id, user_id=user.id)
